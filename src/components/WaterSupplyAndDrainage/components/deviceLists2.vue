    ...
    <el-scrollbar style="height:100%">
      <div class="itemBox">
        <div v-for="(item,i) in DLists" class="item no-select" :style="[i%5!=0? marginL:noMargin,i>4? {marginTop:'0.18rem'}:{marginTop:0}]" :key="i">
          ...
          <img class="img" :src="deviceImages[(item.state==1 || item.state==2)?2:item.state==0?0:1].src" alt="">
...
<script>
  ...
  export default {
    ...
    props:['DLists','deviceImages','deviceImagesSize'],
    name: "deviceLists2",
    /*computed:{
      newDLists:function () {
        let arr = [];
        this.DLists.map((item,i)=>{

          //console.log(eval('('+item.data.daiti+')'))
          let lists = eval('('+item.data.daiti+')').list
          let realState='';
          console.log('lists',lists)
          lists.some((item2,i2,arr)=>{
            //console.log(item2,i2,arr)
            if (item2.type == 0){
              if (item2.min != '' && item2.max != '') {
                if (parseFloat(item2.min) < parseFloat(item2.max)) {
                  if (parseFloat(item.data.value) >= parseFloat(item2.min) && parseFloat(item.data.value) <= parseFloat(item2.max)) {
                    realState = 0;
                    return true;
                  }
                }
                ...
                if (item2.min == '') {
                  if (parseFloat(item.data.value) <= parseFloat(item2.max)) {
                    realState = 0;
                    return true;
                  }
                } else if (item2.max == ''){
                  if (parseFloat(item.data.value) >= parseFloat(item2.min)) {
                    realState = 0;
                    return true;
                  }
                }
          ...
          console.log('状态',realState)
          item.realState = realState;
          arr.push(item)
    ...
    methods: {
      ...
      getProjectInfoLists(sysID,id){
        ...
        this.$http.post('drainage/drainage_device_protectinfo', config, headers).then(res => {
          let data = res.data;
          console.log('系统维保历史列表2', config, res);
          if(data.code==0){
