...
export default {
  ...
  methods:{
    ...
    changeOk(){
      ...
      let headers = {};
      this.$http.post('energy_pc/pc/plan/set', data, headers).then(res => {
        this.loading =false;
        let data0 = res.data;
        console.log('用能预设设置', data, res);
        if(data0.code==0){
    ...
    getData(){
      ...
      this.$http.post('pc/2.0/energy/index/plan_status', data, headers).then(res => {
        this.loading =false;
        let data0 = res.data;
        console.log('用能预设查询', data, res);
        if(data0.code==0){
          let datas = data0.data;

          let query_year_actual = datas.query_year_actual;

          let query_year_plan = datas.query_year_plan;
