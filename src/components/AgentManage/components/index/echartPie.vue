...
export default {
  ...
  methods:{
    drawLine(){
        ...
        function whiteCircle(data){//绘制白色外圈
            
            context.save();
            //context.beginPath();
            context.lineWidth = 2; //设置线宽
            context.strokeStyle = "red";
            // context.arc(centerX, centerY, n4 , 0, Math.PI*2, false);
            // context.closePath(); //路径结束
            // context.stroke(); //绘制

            // context.beginPath();
            // context.arc(centerX, centerY, n3 , 0, Math.PI*2, false);
            // context.closePath(); //路径结束
            // context.stroke(); //绘制

            // context.beginPath();
            // context.arc(centerX, centerY, n2 , 0, Math.PI*2, false);
            // context.closePath(); //路径结束
            // context.stroke(); //绘制
            for(var i=0;i<data.length;i++){
                context.beginPath();
                context.arc(centerX, centerY,data[i], 0, Math.PI*2, false);
                context.stroke();
                context.closePath();
            }
            context.beginPath();
        ...
        function blueCircle(data){//绘制5像素宽的运动外圈
            ...
            context.arc(0, 0, 100 , -Math.PI*4/3, -Math.PI*4/3 +data[0]*rad, false); //用于绘制圆弧context.arc(x坐标，y坐标，半径，起始角度，终止角度，顺时针/逆时针)
            //context.arc(161,25,2,Math.PI*2,0,true);
            context.stroke(); //绘制
            context.closePath(); //路径结束

            for(var i=0;i<data.length;i++){
                context.translate(Math.round(Math.cos((data[i]*3.6+120)*Math.PI/180 )* 70),Math.round(Math.sin((data[i]*3.6+120)*Math.PI/180 )* 70));
